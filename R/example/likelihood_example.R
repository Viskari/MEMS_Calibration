...
# Some global variables have been included to help with debugging.
...
likelihood <- function(trial_set) {
  ...
  # DEBUG: initialize cost function sum
  c_func_sum <- double(1)
  
  # DEBUG: count likelihood calls
  passed <<- passed + 1

  # DEBUG: save the proposed parameter set
  all_proposed[passed, ] <<- trial_set
  ...
  for (ds in datasets) {
    ...
    if (ds %in% c("ed1", "ed2")) {
      ...
      # DEBUG: increase the overall cfunc sum
      c_func_sum <- c_func_sum + sum(((simulated - list_data[[ds]][["obs"]]) /
                                        list_data[[ds]][["unc"]]) ** 2)
    }
    
    # CIDET, LIDET, Tarasov, Mäkinen, Hob3
    ...
    if (ds %in% c("gss")) {
      ...
      # Sum the results over AWENH, measurements are summed 
      simulated <- rowSums(simulated)
  ...
  # DEBUG: save likelihood and cost function
  lh_cf[passed, 1] <<- ll_sum
  lh_cf[passed, 2] <<- c_func_sum
